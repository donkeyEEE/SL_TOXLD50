...
该模块主要用于评估机器学习模型在化学性质预测中的表现。它支持多种任务，例如计算性能指标、使用指纹进行适用域（AD）评估，并提供结果的可视化。
...
from models.AD_FP import ADBase,ADECFP_Tanimoto
class SL_evaluator():
    ...
    def evaluate_with_AD(self,s=0.85,n=1,ADFP:ADBase = ADECFP_Tanimoto):
